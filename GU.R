...
mpath<-"/Users/aliamiryousefi/Desktop/GU-data/"

mpath<-"/Users/aliamiryousefi/Desktop/cyf_files/"
...
pdf(file = "TLS-compact2.pdf", width=10.5, height=10.5)
...
for (j in 17:29){
  ...
  for (i in 1:ltls){
    ...
    points(s$x, s$y, col=rainbow(ltls)[i], pch=19, cex=0.01)
  }
  
}
dev.off()

###gate the Ki67 on the TLS and save the vector of ones for the postive and zero 

for (i in names(ldata)){
  ff<- ldata[[i]]
  f<- subset(ff, ff$tls_id!=0)
  if (dim(f)[1]!=0){
    KiGate <- as.numeric(c(ff$Ki67 > skew_gate(f$Ki67, 1)$cutoff  & ff$tls_id!=0))
    ldata[[i]] <- cbind(ff, KiGate)  
  }
  else {
    KiGate<- numeric(dim(ff)[1])
    ldata[[i]] <- cbind(ff, KiGate)
  }
}
...
for (i in 1:length(values)) {
  ...
  jj<-i
  i<- c(which(names(ldata)%in%high), which(names(ldata)%in%low))[jj]
...
for (i in seq(1, 14, 1)) {
  ...
  p <- ggplot(df, aes(x, y)) +
    geom_point(aes(color = category), size = 0.65) +  # Decreased point size
    geom_segment(aes(x = mean_x, y = mean_y, xend = ica_end_x1, yend = ica_end_y1), 
                 arrow = arrow(type = "closed", length = unit(0.1, "inches")), color = "green", linewidth = 0.5) +
    geom_segment(aes(x = mean_x, y = mean_y, xend = ica_end_x2, yend = ica_end_y2), 
                 arrow = arrow(type = "closed", length = unit(0.1, "inches")), color = "green", linewidth = 0.5) +
    scale_color_manual(values = c("CD20" = "red", "Rest" = "blue")) +
    ggtitle(paste("ICA Trace:", round(10000 * (ica_trace - 2), 0))) +
    theme_minimal()
  
  plots[[length(plots) + 1]] <- p
}

gridExtra::grid.arrange(grobs = plots, ncol = 5, nrow = 4)
...
for (i in seq(50, 55, 1)) {
  ...
  df$category <- ifelse(bitAnd(df$pflag, mpflag[which(markers == immune.markers[1])]) == mpflag[which(markers == immune.markers[1])], "CD20",
                        ifelse(bitAnd(df$pflag, mpflag[which(markers == immune.markers[2])]) == mpflag[which(markers == immune.markers[2])], "CD4",
                               ifelse(bitAnd(df$pflag, mpflag[which(markers == immune.markers[3])]) == mpflag[which(markers == immune.markers[3])], "CD11c", "Rest")))
  ...
  p <- ggplot(df, aes(x, y)) +
    ...
    geom_segment(aes(x = mean_x, y = mean_y, xend = ica_end_x2, yend = ica_end_y2), 
                 arrow = arrow(type = "closed", length = unit(0.1, "inches")), color = "green", linewidth = 0.5) +
    scale_color_manual(values = c("CD20" = "red", "CD4" = "yellow", "CD11c" = "blue", "Rest" = "gray")) +
    ggtitle(paste("ICA Trace:", round(10000 * (ica_trace - 2), 0))) +
    theme_minimal()
...
gridExtra::grid.arrange(grobs = plots, ncol = 3, nrow = 2)
...
## now there are two type of phenotyping, one is to get the phenotype list for pehnotypes and 
#their positive and negative, then form a vector of "others", and in that for all the phenotypes,
#start from the most abondant one and fill in overlaying on each other, so the smaller ones 
#will alwasy get the priority of visibility,
#this is to avoid the overblowing of the vector above the lenght of the data. 

## the second method is only to get the side bar plots for every thing that is positive 
#showing its frequcny which would be resulting to more than one due to inevitable 
#defficieny with 
...
for (i in 1:length(names(ldata))){
 d<- ldata[[i]]
 phen_vec<- rep("Others", length(d$pflag))
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="SMA")]) == mpflag[which(markers=="SMA")]] <- "Myfibroblasts"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD31")]) == mpflag[which(markers=="CD31")]] <- "Endothelial cells"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="HLADR")]) == mpflag[which(markers=="HLADR")]] <- "Antigen-presenting cells"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD15")]) == mpflag[which(markers=="CD15")]] <- "Neutrophils"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD20")]) == mpflag[which(markers=="CD20")]] <- "B cells"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD3d")]) == mpflag[which(markers=="CD3d")]] <- "T cells"
 phen_vec[bitAnd(d$pflag, (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD4")])) == (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD4")])] <- "CD4 T cells"
 phen_vec[bitAnd(d$pflag, (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD8a")])) == (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD8a")])] <- "CD8 T cells"
 phen_vec[bitAnd(d$pflag, (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD4")] + mpflag[which(markers=="FOXP3")])) == (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD4")] + mpflag[which(markers=="FOXP3")])] <- "Tregs"
 phen_vec[bitAnd(d$pflag, (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD8a")] + mpflag[which(markers=="GranzymeB")])) == (mpflag[which(markers=="CD3d")] + mpflag[which(markers=="CD8a")] + mpflag[which(markers=="GranzymeB")])] <- "Cytotoxic T cells"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD11b")]) == mpflag[which(markers=="CD11b")] | bitAnd(d$pflag, mpflag[which(markers=="CD68")]) == mpflag[which(markers=="CD68")] | bitAnd(d$pflag, mpflag[which(markers=="CD11c")]) == mpflag[which(markers=="CD11c")] | bitAnd(d$pflag, mpflag[which(markers=="CD163")]) == mpflag[which(markers=="CD163")]] <- "Myeloid cells"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD11b")]) == mpflag[which(markers=="CD11b")] | bitAnd(d$pflag, mpflag[which(markers=="CD68")]) == mpflag[which(markers=="CD68")] & bitAnd(d$pflag, mpflag[which(markers=="CD11c")]) != mpflag[which(markers=="CD11c")]] <- "Macrophages"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD11b")]) == mpflag[which(markers=="CD11b")] | bitAnd(d$pflag, mpflag[which(markers=="CD68")]) == mpflag[which(markers=="CD68")] & bitAnd(d$pflag, mpflag[which(markers=="CD11c")]) != mpflag[which(markers=="CD11c")] & bitAnd(d$pflag, mpflag[which(markers=="CD163")]) == mpflag[which(markers=="CD163")]] <- "M2 Macrophages"
 phen_vec[bitAnd(d$pflag, mpflag[which(markers=="CD11b")]) == mpflag[which(markers=="CD11b")] | bitAnd(d$pflag, mpflag[which(markers=="CD68")]) == mpflag[which(markers=="CD68")] & bitAnd(d$pflag, mpflag[which(markers=="CD11c")]) == mpflag[which(markers=="CD11c")]] <- "Dendritic cells"
 d<- cbind(d, phen_vec)
 ldata[[i]] <- d
}
...
lpheprop<- matrix(0, nrow = length(low), ncol = length(phen_vec))
rownames(lpheprop)<- low
colnames(lpheprop)<- phen_vec
ic<- 1
for (i in low){
  d<- ldata[[i]]
  den<- length(d$phen_vec) 
  jc<-1
  for (j in phen_vec){
    lpheprop[ic, jc]<- sum(d$phen_vec==j)/ den
    jc<- jc+1
  }
  ic<- ic+1
}

hpheprop<- matrix(0, nrow = length(high), ncol = length(phen_vec))
rownames(hpheprop)<- high
colnames(hpheprop)<- phen_vec
ic<- 1
for (i in high){
  d<- ldata[[i]]
  den<- length(d$phen_vec) 
  jc<-1
  for (j in phen_vec){
    hpheprop[ic, jc]<- sum(d$phen_vec==j)/ den
    jc<- jc+1
  }
  ic<- ic+1
}
...
# Combine mean values and p-values
mean_values <- mean_values %>%
  left_join(prop_test_results, by = "Cell_Type")
# Create the bar plot with customized colors and without outliers
ggplot(combined_data, aes(x = Cell_Type, y = Value, fill = Group)) +
 ...
 # geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.9), alpha = 0.5) +
  ...
  scale_fill_manual(values = c("High" = "#7570b3", "Low" = "#1b9e77")) +
  labs(title = "Average Proportions of Cell Types",
...
ggplot(combined_data, aes(x = Cell_Type, y = Value, fill = Group)) +
  ...
  #geom_boxplot(position = position_dodge(width = 0.9), alpha = 0.3, outlier.shape = NA) +
  scale_fill_manual(values = c("High" = "#7570b3", "Low" = "#1b9e77")) +
  labs(title = "Average Proportions of Cell Types on the Tumor",
       x = "Cell Type", y = "Proportion", fill = "Group") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(combined_data, aes(x = Cell_Type, y = Value, fill = Group)) +
  ...
  #geom_boxplot(position = position_dodge(width = 0.9), alpha = 0.3, outlier.shape = NA) +
  scale_fill_manual(values = c("High" = "#7570b3", "Low" = "#1b9e77")) +
  labs(title = "Average Proportions of Cell Types on the Stroma",
       x = "Cell Type", y = "Proportion", fill = "Group") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
hborder<- matrix(0, nrow = length(high), ncol = length(37:126))
rownames(hborder)<- high
colnames(hborder)<- names(ldata[[1]])[37:126]
...
lborder<- matrix(0, nrow = length(low), ncol = length(37:126))
rownames(lborder)<- low
colnames(lborder)<- names(ldata[[1]])[37:126]
